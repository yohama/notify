...
void header(const char *string, uint16_t color)
{
    ...
    if (PORT != 80) M5.Lcd.printf(":%d", PORT);
}
...
void cmd_run(String cmd) {
  ...
  if(indexNum != -1){
      ...
      M5.Lcd.fillRect(0,80, 320, 160, bg_color);
      delay(500);
      if (cmd_str == "start"){
        if (!mute){
          sb.beep(volume, C4, 100);
          sb.beep(volume, D4, 100);
          sb.beep(volume, E4, 100);
        }
        bg_color = TFT_YELLOW;
      }
      if (cmd_str == "done"){
...
void setup() {
  ...
  header("Notification", TFT_BLACK);

  sb.init();
  if (!mute) sb.beep(volume, A4, 100);
}
...
void loop() {
  ...
  if(M5.BtnB.wasPressed()){
    if (mute) {
      mute = false;
      sb.beep(volume, A4, 100);
    } else {
      mute = true;
    }
  }
  if(M5.BtnC.wasPressed() && !mute){
    if (volume < 20)
      volume++;
    if (!mute) sb.beep(volume, A4, 100);
  }
  M5.Lcd.setCursor(0, 220, 4);
  if (mute) {
    M5.Lcd.print("Volume: - -  ");
  } else {
    M5.Lcd.printf("Volume: %02d", volume);
  }
}
